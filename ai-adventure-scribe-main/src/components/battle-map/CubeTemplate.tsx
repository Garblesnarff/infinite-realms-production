/**
 * Cube Template Component
 *
 * Renders a cube/square-shaped area of effect template for spells like Thunderwave.
 * Features:
 * - Side length input
 * - Snap to grid
...
import { getCubePoints, getTokensInCube, getAffectedGridSquares } from '@/utils/template-calculations';
...
export interface CubeTemplateProps {
  /** Template data */
...
export const CubeTemplate: React.FC<CubeTemplateProps> = ({
  template,
  gridSize,
  tokens = [],
  draggable = true,
  ...
  const size = template.distance;
  const rotation = template.direction || 0;

  // Calculate cube geometry
  const cubePoints = useMemo(
    () => getCubePoints(origin, size, gridSize, rotation),
    [origin, size, gridSize, rotation]
  );

  // Create shape geometry
  const shapeGeometry = useMemo(() => {
    const shape = new THREE.Shape();
    if (cubePoints.length === 0) return null;

    shape.moveTo(cubePoints[0].x, cubePoints[0].y);
    for (let i = 1; i < cubePoints.length; i++) {
      shape.lineTo(cubePoints[i].x, cubePoints[i].y);
    }
    shape.closePath();
  ...
  const affectedTokens = useMemo(() => {
    ...
    return getTokensInCube(origin, size, tokens, gridSize, rotation);
  }, [origin, size, tokens, gridSize, rotation, highlightTokens]);
  ...
  const handlePointerMove = (event: ThreeEvent<PointerEvent>) => {
    ...
    if (isDragging && onDrag) {
      ...
      // Snap to 45-degree increments for cubes
  ...
  return (
    <group ref={groupRef} name={`cube-template-${template.id}`}>
      ...
      <line>
        <bufferGeometry>
          <bufferAttribute
            ...
            count={cubePoints.length + 1}
            array={
              new Float32Array([
                ...cubePoints.flatMap((p) => [p.x, p.y, 0.21]),
                cubePoints[0].x,
                cubePoints[0].y,
                0.21,
              ])
        ...
        <line>
          <bufferGeometry>
            <bufferAttribute
              ...
              array={
                new Float32Array([
                  ...cubePoints.flatMap((p) => [p.x, p.y, 0.22]),
                  cubePoints[0].x,
                  cubePoints[0].y,
                  0.22,
                ])
      ...
      {rotatable && rotation !== 0 && (
        <DirectionIndicator origin={origin} rotation={rotation} size={size} gridSize={gridSize} color={borderColor} />
      )}
      ...
      <Html position={[origin.x, origin.y - 20, 0.3]} center>
        ...
        >
          {template.spellName || 'Cube'}: {size}ft cube
...
const DirectionIndicator: React.FC<DirectionIndicatorProps> = ({ origin, rotation, size, gridSize, color }) => {
  const rotationRad = (rotation * Math.PI) / 180;
  const length = (size / 5) * gridSize * 0.3;

  const endPoint = {
    x: origin.x + Math.cos(rotationRad) * length,
    y: origin.y + Math.sin(rotationRad) * length,
  };
