/**
 * Cone Template Component
 *
 * Renders a cone-shaped area of effect template for spells like Burning Hands.
 * Features:
 * - 90° cone (default) or custom angle
 * - Snap origin to grid
 * - Rotate to face direction
 * - Show affected squares
 * - Calculate from caster position
 */
...
import { getConePoints, getTokensInCone, getAffectedGridSquares } from '@/utils/template-calculations';
...
export interface ConeTemplateProps {
  /** Template data */
...
export const ConeTemplate: React.FC<ConeTemplateProps> = ({
  template,
  gridSize,
  tokens = [],
  draggable = true,
  ...
  const angle = template.angle || 90;

  // Calculate cone geometry
  const conePoints = useMemo(
    () => getConePoints(origin, template.direction, template.distance, angle, gridSize),
    [origin, template.direction, template.distance, angle, gridSize]
  );
  ...
  const shapeGeometry = useMemo(() => {
    ...
    shape.moveTo(conePoints[0].x, conePoints[0].y);
    for (let i = 1; i < conePoints.length; i++) {
      shape.lineTo(conePoints[i].x, conePoints[i].y);
    }
    shape.closePath();
  ...
  const affectedTokens = useMemo(() => {
    ...
    return getTokensInCone(origin, template.direction, angle, template.distance, tokens, gridSize);
  }, [origin, template.direction, angle, template.distance, tokens, gridSize, highlightTokens]);
  ...
  const handlePointerMove = (event: ThreeEvent<PointerEvent>) => {
    ...
    if (isDragging && onDrag) {
      ...
      onRotate(template, newDirection);
    }
  };
  ...
  return (
    <group ref={groupRef} name={`cone-template-${template.id}`}>
      ...
      <line>
        <bufferGeometry>
          <bufferAttribute
            ...
            count={conePoints.length + 1}
            array={
              new Float32Array([
                ...conePoints.flatMap((p) => [p.x, p.y, 0.21]),
                conePoints[0].x,
                conePoints[0].y,
                0.21,
              ])
        ...
        <line>
          <bufferGeometry>
            <bufferAttribute
              ...
              array={
                new Float32Array([
                  ...conePoints.flatMap((p) => [p.x, p.y, 0.22]),
                  conePoints[0].x,
                  conePoints[0].y,
                  0.22,
                ])
      ...
      {rotatable && (
        <DirectionArrow
          ...
          length={template.distance * (gridSize / 5)}
          color={borderColor}
        />
      )}
      ...
      <Html position={[origin.x, origin.y - 20, 0.3]} center>
        ...
        >
          {template.spellName || 'Cone'}: {template.distance}ft
...
const DirectionArrow: React.FC<DirectionArrowProps> = ({ origin, direction, length, color }) => {
  const directionRad = ((direction - 90) * Math.PI) / 180;
  const endPoint = {
    x: origin.x + Math.cos(directionRad) * length * 0.3,
    y: origin.y + Math.sin(directionRad) * length * 0.3,
  };
  ...
  return (
    <group>
      ...
      <mesh position={[endPoint.x, endPoint.y, 0.31]}>
