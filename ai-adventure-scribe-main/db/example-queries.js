...
import { blogPosts, blogAuthors, blogCategories, blogPostCategories } from './schema';
...
export async function searchPosts(query) {
    ...
    return await db
        .select()
        .from(blogPosts)
        .where(and(eq(blogPosts.status, 'published'), sql `${blogPosts.title} ILIKE ${searchPattern} OR ${blogPosts.content} ILIKE ${searchPattern}`))
        .orderBy(desc(blogPosts.publishedAt));
}
/**
 ...
 */
export async function createPostWithCategories(postData, categoryIds) {
 ...
 */
export async function getRecentPostsByAuthor(authorId, limit = 5) {
    return await db
        .select()
        .from(blogPosts)
